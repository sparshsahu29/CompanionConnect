...
import { getMessages } from '../api/chatApi'; // We only need this for the initial history load now
import { useAuth } from '../context/AuthContext';
import { Send, ChevronLeft, Info, Phone, Video, MoreVertical, Smile } from 'lucide-react';

// Initialize the socket pipeline outside the component to prevent re-renders from killing the connection
...
const Chat = () => {
  ...
  useEffect(() => {
    const initializeChat = async () => {
      try {
        // 1. Fetch historical messages via standard HTTP
    ...
    // 2. Connect to the WebSocket and join the specific booking room
    socket.connect();
    ...
    // 4. Cleanup: Disconnect when the user leaves the chat page
    return () => {
  ...
  const handleSend = (e) => {
    ...
    // Instantly push the message down the socket pipeline
    socket.emit('send_message', {
      bookingId,
      message: newMessage
    });

    // Clear the input immediately for a snappy user experience
    setNewMessage('');
  };
