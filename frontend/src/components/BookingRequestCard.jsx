...
const BookingRequestCard = ({ request, onAccept, onDecline, onOpenChat }) => {
  const requesterName = request.userName || request.clientName || 'Unknown';
  const bookingDate = request.date || request.bookingDate || 'Not specified';
  const bookingTime = request.time || request.timeSlot || 'Not specified';
        ...
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center overflow-hidden border-2 border-white shadow-sm">
            {request.userPhoto ? (
              ...
              <span className="text-indigo-600 font-bold text-lg">{initial}</span>
            )}
          </div>
          <div>
            <h4 className="font-bold text-gray-900">{requesterName}</h4>
            <span className="text-xs text-gray-500">Sent {createdAt}</span>
          </div>
        </div>
