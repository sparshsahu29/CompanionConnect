...
import { updateCompanionSettings, getMyCompanionProfile } from '../api/companionApi';
    ...
    const fetchData = async () => {
      try {
        const [reqData, companionData] = await Promise.all([
          getBookingRequests(),
          getMyCompanionProfile().catch(() => null)
        ]);
  ...
  const handleSettingsSubmit = async (e) => {
    ...
    try {
      const formattedSettings = {
        hourlyRate: settings.hourlyRate === '' ? null : Number(settings.hourlyRate),
        availableDates: settings.availableDates.split(',').map(d => d.trim()).filter(Boolean),
        availableTimeSlots: settings.availableTimeSlots.split(',').map(t => t.trim()).filter(Boolean),
        isActive: settings.isActive
      };
      const result = await updateCompanionSettings(formattedSettings);
      if (result.companion) {
        setSettings({
          hourlyRate: result.companion.hourlyRate || '',
          availableDates: result.companion.availableDates?.join(', ') || '',
          availableTimeSlots: result.companion.availableTimeSlots?.join(', ') || '',
          isActive: result.companion.isActive !== false,
        });
      }
      setSaveSuccess(true);
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {requests.map(request => (
                    <BookingRequestCard 
                      ...
                      onDecline={() => onHandleRequest(request.id, 'reject')}
